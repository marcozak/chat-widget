...
">
        ...
        <button
            class="bg-transparent border-transparent absolute top-4 left-4 z-10 flex items-center"
            @click="$emit('go-back')"
            >
        ...
        <p class="absolute top-4dot5 left-1/2  text-white font-PeugeotNew text-xs text-center whitespace-nowrap">
            ...
            >
                ...
                <div class="h-[480px] mt-10 flex justify-center">
                    <div class="h-[120px] w-[100%] absolute top-2 flex items-center justify-center">
                        <img :src="`${assetBaseUrl}/${slide.imageUrl}`" :alt="slide.car" class="">
                    </div>
                    <div class="flex grow justify-center items-center h-full overflow-hidden w-full pb-20">
                        <div class="bg-blue h-[410px] w-full rounded-3xl flex flex-col mt-10 pt-16">
                            ...
                            <div class="flex flex-col justify-center items-center h-[220px] pb-2 pt-2 px-6 gap-2.5">
                                <button v-if="slide.showRoomUrl" class="bg-white border-transparent rounded-full w-full py-3 font-PeugeotNewBold text-[10px] text-blue">
                                    <a :href="slide.showRoomUrl" target="_blank" rel="noopener noreferrer">
                                        {{ translations.carousel.buttons.showRoomLabel }}
                                    </a>
                                </button>
                                <button v-if="slide.configureUrl" class="bg-white border-transparent rounded-full w-full py-3 font-PeugeotNewBold text-[10px] text-blue">
                                    <a :href="slide.configureUrl" target="_blank" rel="noopener noreferrer">
                                        {{ translations.carousel.buttons.configureLabel }}
                                    </a>
                                </button>
                                <button v-if="slide.offersFormUrl" class="bg-white border-transparent rounded-full w-full py-3 font-PeugeotNewBold text-[10px] text-blue">
                                    <a :href="slide.offersFormUrl" target="_blank" rel="noopener noreferrer">
                                        {{ translations.carousel.buttons.offersLabel }}
                                    </a>
                                </button>
                                <button v-if="slide.testDriveUrl" class="bg-white border-transparent rounded-full w-full py-3 font-PeugeotNewBold text-[10px] text-blue">
